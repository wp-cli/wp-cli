Feature: Utilities that do NOT depend on WordPress code
  ...
  Scenario: Check that `Utils\run_mysql_command()` can return data and errors if requested
    ...
    And a eval-run-mysql-success.php file:
      """
      <?php
      list( $stdout, $stderr, $exit_code ) = WP_CLI\Utils\run_mysql_command( '{MYSQL_BINARY} --no-defaults', [ 'user' => '{DB_USER}', 'pass' => '{DB_PASSWORD}', 'host' => '{DB_HOST}', 'execute' => 'SHOW DATABASES;' ], null, false );
      fwrite( STDOUT, strtoupper( $stdout ) );
      fwrite( STDERR, strtoupper( $stderr ) );
      """
    And a eval-run-mysql-error.php file:
      """
      <?php
      list( $stdout, $stderr, $exit_code ) = WP_CLI\Utils\run_mysql_command( '{MYSQL_BINARY} --no-defaults', [ 'user' => '{DB_USER}', 'pass' => '{DB_PASSWORD}', 'host' => '{DB_HOST}', 'execute' => 'broken query' ], null, false );
      fwrite( STDOUT, strtoupper( $stdout ) );
      fwrite( STDERR, strtoupper( $stderr ) );
      """

    When I run `wp eval-file eval-run-mysql-success.php --skip-wordpress`
    ...
    When I try `wp eval-file eval-run-mysql-error.php --skip-wordpress`
    Then STDOUT should be empty
