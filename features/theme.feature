Feature: Manage WordPress themes
  ...
  Scenario: Installing and deleting theme
    ...
    When I run `wp theme status hexa`
    ...
    When I run `wp theme path hexa`
    Then STDOUT should contain:
      """
      /themes/hexa/style.css
      """

    When I run `wp option get stylesheet`
    Then save STDOUT as {PREVIOUS_THEME}

    When I run `wp theme activate hexa`
    Then STDOUT should contain:
      """
      Success: Switched to 'Hexa' theme.
      """

    When I try `wp theme delete hexa`
    Then STDERR should be:
      """
      Warning: Can't delete the currently active theme: hexa
    ...
    Then STDERR should contain:
      """
      The 'hexa' theme could not be found.
      """
  ...
  Scenario: Install a theme, activate, then force install an older version of the theme
    ...
    When I run `wp theme install hexa --version=1.0.1`
    Then STDOUT should not be empty

    When I run `wp theme list`
    Then STDOUT should be a table containing rows:
      | name  | status   | update    | version   |
      | hexa  | inactive | available | 1.0.1     |

    When I run `wp theme activate hexa`
    Then STDOUT should not be empty

    When I run `wp theme install hexa --version=1.0 --force`
    Then STDOUT should not be empty

    When I run `wp theme list`
    Then STDOUT should be a table containing rows:
      | name  | status   | update    | version   |
      | hexa  | active   | available | 1.0       |
    ...
    Then STDOUT should contain:
      """
      Hexa updated successfully from version 1.0 to version
      """
  ...
  Scenario: Get the path of an installed theme
    ...
    When I run `wp theme path hexa --dir`
    Then STDOUT should contain:
       """
       wp-content/themes/hexa
       """
  ...
  Scenario: Activate an already active theme
    ...
    When I try `wp theme activate hexa`
    Then STDERR should be:
      """
      Warning: The 'Hexa' theme is already active.
      """
  ...
  Scenario: Enabling and disabling a theme without multisite
    ...
    When I try `wp theme enable hexa`
    Then STDERR should be:
      """
      Error: This is not a multisite install.
      """

    When I try `wp theme disable hexa`
    Then STDERR should be:
      """
      Error: This is not a multisite install.
      """
  ...
  Scenario: Install a theme, then update to a specific version of that theme
    ...
    When I run `wp theme install hexa --version=1.0`
    Then STDOUT should not be empty

    When I run `wp theme update hexa --version=1.0.1`
    Then STDOUT should not be empty
