Feature: `wp cli completions` tasks
  ...
  Scenario: Bash Completion for commands for added by WordPress plugin
    Given a WP install
    And a wp-content/mu-plugins/command-bash-completion.php file:
      """
      <?php
      /**
      ...
      WP_CLI::add_command( 'my-command', 'My_Command_Namespace' );
      """
    ...
    When I run `wp cli completions --line="wp m" --point=100`
    Then STDOUT should contain:
      """
      my-command
      """
